  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {pending && (
          <>
            ...
            <span className="mono truncate" style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--accent-strong)', flex: 1 }}>
              {id.staged}
            </span>
          </>
        )}
      ...
      <div style={{ display: 'flex', gap: 4, flex: '0 0 auto' }}>
        ...
        {pending
          ? <button className="btn btn-ghost btn-sm" onClick={() => onClear(id.key)}>撤销</button>
          : <button className="btn btn-soft btn-sm" disabled={id.locked} onClick={() => onStage(id.key)}>
...
export default function DeviceIDs({
  ...
  applyAll, resetAll, stageAll, onCopy,
}: DeviceIDsProps) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 'var(--gap)' }}>
        <div style={{ position: 'relative', flex: 1, maxWidth: 320 }}>
          ...
          <input
            ...
            placeholder="搜索标识或值…"
            style={{
              width: '100%', height: 36, padding: '0 12px 0 34px', borderRadius: 'var(--radius-sm)',
