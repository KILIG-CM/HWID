...
function InfoRow({ id }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 0", borderBottom: "1px solid var(--hairline)" }}>
      <span style={{ color: "var(--text-faint)", display: "grid", placeItems: "center", width: 18 }}><Icon name={id.icon} size={15} /></span>
      ...
      {id.locked
        ...
        : modified
          ? <span className="tag tag-accent"><span className="dot" style={{ background: "var(--accent)" }}></span>已修改</span>
          : <span className="tag tag-ok"><span className="dot" style={{ background: "var(--ok)" }}></span>原始</span>}
    </div>
  );
}

function Dashboard({ identifiers, snapshots, onGoto, fingerprint }) {
  const modified = identifiers.filter(i => i.value !== i.original).length;
  const locked = identifiers.filter(i => i.locked).length;
  const groups = ["系统标识", "网络", "硬件"];
    ...
    <div className="page page-anim">
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden", marginBottom: "var(--gap)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 20, padding: "22px 24px",
          ...
          <div style={{ width: 56, height: 56, borderRadius: 16, flex: "0 0 auto", display: "grid", placeItems: "center",
          ...
          {groups.map(g => (
            <div className="card" key={g}>
              <div className="card-title" style={{ marginBottom: 6 }}>
                <span className="ct-icon"><Icon name={g === "网络" ? "network" : g === "硬件" ? "cpu" : "shield"} size={15} /></span>
                {g}
              </div>
